...
// Windows-specific writeFails test with adapted chmod expectations
func writeFails(t *testing.T, fsys vroot.Fs) {
	// Test filesystem-level write operations that should fail

	// Create should fail
	f, err := fsys.Create("should_fail.txt")
	if err == nil {
		f.Close()
	...
	f, err = fsys.OpenFile("file1.txt", os.O_RDWR, 0o644)
	if err == nil {
		f.Close()
		t.Error("OpenFile with O_RDWR should have failed on read-only filesystem")
	}

	// Test Windows-specific chmod behavior
	...
	err = fsys.MkdirAll(filepath.FromSlash("new/deep/dir"), 0o755)
	if err == nil || errors.Is(err, fs.ErrNotExist) {
	...
	// Test Windows-specific file chmod behavior
